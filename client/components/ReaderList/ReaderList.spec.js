...
test('ReaderList take a snapshot', () => {
  const component = shallow(<UnwrappedReaderList fetchArticlesForFeedsFromDb={()=>{}} articles={tempData} articlesFound={tempData["58f7a4b09f861f7da0e08542"]}/>);
  const tree = shallowToJson(component);
  expect(tree).toMatchSnapshot();
});
test('ReaderList should render a ReaderListItem for each article', () => {

  const component = render(<Provider store={store}><UnwrappedReaderList fetchArticlesForFeedsFromDb={()=>{}} articlesFound={tempData["58f7a4b09f861f7da0e08542"]} articles={tempData} currentFeed={""}/></Provider>);
